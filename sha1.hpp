...
#undef SHA1_ROUND_4
   ...
   public:
    ...
    sha1& add(const char* text) { return add(text, strlen(text)); }
    ...
    const sha1& print_base64(char* base64, bool zero_terminate = true) const {
        ...
        uint32_t triples[7] = {
            ...
            ((state[0] & 0x000000ff) << 2 * 8) |
                ((state[1] & 0xffff0000) >> 2 * 8),
            ((state[1] & 0x0000ffff) << 1 * 8) |
                ((state[2] & 0xff000000) >> 3 * 8),
            ((state[2] & 0x00ffffff) << 0 * 8),
            ((state[3] & 0xffffff00) >> 1 * 8),
            ((state[3] & 0x000000ff) << 2 * 8) |
                ((state[4] & 0xffff0000) >> 2 * 8),
